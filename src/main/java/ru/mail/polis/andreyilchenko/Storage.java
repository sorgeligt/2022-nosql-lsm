...
class Storage implements Closeable {
    ...
    private static final Comparator<Path> fileComparator
            = Comparator.comparingInt(Storage::getPriorityFile).reversed();
    ...
    static Storage load(Config config) throws IOException {
        ...
        try (Stream<Path> streamFiles = Files.list(basePath)) {
            List<Path> paths = checkExistingAndSortedList(streamFiles);
            if (!paths.isEmpty()) {
                maxPriorityFile = getPriorityFile(paths.get(0));
            }
            for (Path path : paths) {
                try {
    ...
    private static List<Path> checkExistingAndSortedList(Stream<Path> streamFiles) {
        return streamFiles
                .filter(path -> path.toString().endsWith(FILE_EXT))
                .sorted(fileComparator).toList();
    }
    ...
    public List<Iterator<Entry<MemorySegment>>> sstablesIterate(MemorySegment keyFrom, MemorySegment keyTo) {
        List<Iterator<Entry<MemorySegment>>> iterators = new ArrayList<>(sstables.size());
        sstables.forEach(x -> iterators.add(iterate(x, keyFrom, keyTo)));
        return iterators;
    }
